    ...
    public partial class SystemManagementViewModel : ObservableObject
    {
        ...
        private async Task ServiceActionAsync(string? action)
        {
            ...
            if (actionName is "start" or "stop" or "restart")
            {
                ...
                if (Confirm == null || !await Confirm($"确定要{verb}服务 {s.Name} 吗？", $"{verb}服务")) return;
            }
        ...
        private async Task KillProcessAsync(int signal)
        {
            ...
            if (Confirm == null || !await Confirm($"确定要对进程 {SelectedProcess.Pid} ({SelectedProcess.Cmd}) 发送信号 {signal} 吗？", "结束进程")) return;
        ...
        private async Task KillAllAsync(int signal)
        {
            ...
            if (Confirm == null || !await Confirm($"确定要结束所有名为 \"{procName}\" 的进程吗？\n信号: {signal}", "全部结束")) return;
        ...
        private async Task KillNetAsync(int signal)
        {
            ...
            if (Confirm == null || !await Confirm($"确定要结束进程 {c.Pid} ({c.Program}) 吗？\n信号: {signal}", "结束进程")) return;
        ...
        private async Task ToggleCronAsync(bool enabled)
        {
            ...
            if (Confirm == null || !await Confirm($"确定要{action}该计划任务吗？\n\n{SelectedCronJob.Schedule}\n{SelectedCronJob.Command}", $"{action}任务")) return;
        ...
        {
            if (SelectedCronJob == null) return;
            if (Confirm == null || !await Confirm($"确定要删除该计划任务吗？\n\n{SelectedCronJob.Schedule}\n{SelectedCronJob.Command}", "删除任务")) return;
